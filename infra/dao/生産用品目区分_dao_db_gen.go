...
func (d *daoDb生産用品目区分) init() (err error) {
...
func (d daoDb生産用品目区分) Dt() ([]*Dto生産用品目区分, error) {
...
func (d daoDb生産用品目区分) GetBy(id Id) (dr *Dto生産用品目区分, err error) {
...
func (d daoDb生産用品目区分) SelectW(wb Wb生産用品目区分) ([]*Dto生産用品目区分, error) {
	where := wb.build()
...
func (d daoDb生産用品目区分) Count() (cnt int64, err error) {
	return d.CountW(NewWb生産用品目区分())
}
func (d daoDb生産用品目区分) CountW(wb Wb生産用品目区分) (cnt int64, err error) {
	where := wb.build()
	prms, exists := where.Params()
	if exists {
		err = d.db.QueryRow(fmt.Sprintf(sqlSelect生産用品目区分ForAggregation, "count(\"ID\")", where.String()), prms...).Scan(&cnt)
		if err != nil {
			err = xerrors.Errorf(": %w", err)
			return
		}
		...
		err = d.db.QueryRow(fmt.Sprintf(sqlSelect生産用品目区分ForAggregation, "count(\"ID\")", "")).Scan(&cnt)
		if err != nil {
			err = xerrors.Errorf(": %w", err)
			return
		}
...
func (d daoDb生産用品目区分) Min(fld fld生産用品目区分) (min int64, err error) {
	return d.MinW(fld, NewWb生産用品目区分())
}
func (d daoDb生産用品目区分) MinW(fld fld生産用品目区分, wb Wb生産用品目区分) (min int64, err error) {
	where := wb.build()
	prms, exists := where.Params()
	var x sql.NullInt64
	if exists {
		err = d.db.QueryRow(fmt.Sprintf(sqlSelect生産用品目区分ForAggregation, fmt.Sprintf("min(%s)", strconv.Quote(string(fld))), where.String()), prms...).Scan(&x)
	} else {
		err = d.db.QueryRow(fmt.Sprintf(sqlSelect生産用品目区分ForAggregation, fmt.Sprintf("min(%s)", strconv.Quote(string(fld))), where.String())).Scan(&x)
	}
	if err != nil {
		err = xerrors.Errorf(": %w", err)
		return
...
func (d daoDb生産用品目区分) Max(fld fld生産用品目区分) (max int64, err error) {
	return d.MaxW(fld, NewWb生産用品目区分())
}
func (d daoDb生産用品目区分) MaxW(fld fld生産用品目区分, wb Wb生産用品目区分) (max int64, err error) {
	where := wb.build()
	prms, exists := where.Params()
	var x sql.NullInt64
	if exists {
		err = d.db.QueryRow(fmt.Sprintf(sqlSelect生産用品目区分ForAggregation, fmt.Sprintf("max(%s)", strconv.Quote(string(fld))), where.String()), prms...).Scan(&x)
	} else {
		err = d.db.QueryRow(fmt.Sprintf(sqlSelect生産用品目区分ForAggregation, fmt.Sprintf("max(%s)", strconv.Quote(string(fld))), where.String())).Scan(&x)
	}
	if err != nil {
		err = xerrors.Errorf(": %w", err)
		return
...
func (d daoDb生産用品目区分) Insert(dr *Dto生産用品目区分) (err error) {
	err = d.db.QueryRow(sqlInsert生産用品目区分, dr.Fldコード, dr.Fld名称, dr.Fld何かのフラグ1, dr.Fld何かのフラグ2).Scan(&dr.FldID)
	if err != nil {
		err = xerrors.Errorf(": %w", err)
		return
	}
...
func (d daoDb生産用品目区分) MultiInsert(dt []*Dto生産用品目区分) (err error) {
	cs := a.Chunk(dt, 1000)
	for _, c := range cs {
		...
		_, err = d.db.Exec(fmt.Sprintf(sqlMultiInsert生産用品目区分, strings.Join(vals, ",")), args...)
		if err != nil {
			err = xerrors.Errorf(": %w", err)
			return
		}
...
func (d daoDb生産用品目区分) UpdateBy(dr *Dto生産用品目区分) (cnt int64, err error) {
	if dr.Ub.Count() == 0 {
		dr.rowState = UnChanged
		return
	}
...
func (d daoDb生産用品目区分) UpdateW(ub *ub生産用品目区分, wb Wb生産用品目区分) (cnt int64, err error) {
...
func (d daoDb生産用品目区分) DeleteBy(dr *Dto生産用品目区分) (cnt int64, err error) {
...
func (d daoDb生産用品目区分) DeleteW(wb Wb生産用品目区分) (cnt int64, err error) {
	where := wb.build()
	prms, exists := where.Params()
	if exists {
		sql := fmt.Sprintf(sqlDelete生産用品目区分, where.String())
		...
		result, errExec := d.db.Exec(sqlTruncate生産用品目区分)
		if errExec != nil {
