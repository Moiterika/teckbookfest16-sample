...
type ubEnum受払区分 struct {
	config          []updateBuilderKvPair
	keys            map[string]int
	isBuildComplete bool
}

func NewUbEnum受払区分() *ubEnum受払区分 {
	return &ubEnum受払区分{
...
func (ub *ubEnum受払区分) Set(field fldEnum受払区分, val interface{}) *ubEnum受払区分 {
	k := string(field)
...
func (ub *ubEnum受払区分) Clear() {
	ub.config = make([]updateBuilderKvPair, 0)
	ub.keys = make(map[string]int)
	ub.isBuildComplete = false
}
func (ub *ubEnum受払区分) Count() int {
	return len(ub.config)
}
func (ub *ubEnum受払区分) build(wb WbEnum受払区分) (s string, w string, execArgs []interface{}) {
	if ub.Count() == 0 {
		// 更新項目なし
		return
	}
...
func (ub *ubEnum受払区分) copyMap() map[string]interface{} {
