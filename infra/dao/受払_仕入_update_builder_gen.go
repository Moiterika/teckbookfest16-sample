...
func (ub *ub受払仕入) Set(field fld受払仕入, val interface{}) *ub受払仕入 {
	k := string(field)
...
func (ub *ub受払仕入) Clear() {
	ub.config = make([]updateBuilderKvPair, 0)
	ub.keys = make(map[string]int)
	ub.isBuildComplete = false
}
func (ub *ub受払仕入) Count() int {
	return len(ub.config)
}
func (ub *ub受払仕入) build(wb Wb受払仕入) (s string, w string, execArgs []interface{}) {
	if ub.Count() == 0 {
		// 更新項目なし
		return
	}
...
func (ub *ub受払仕入) copyMap() map[string]interface{} {
