...
func (ub *ub受払投入実績) Set(field fld受払投入実績, val interface{}) *ub受払投入実績 {
	k := string(field)
...
func (ub *ub受払投入実績) Clear() {
	ub.config = make([]updateBuilderKvPair, 0)
	ub.keys = make(map[string]int)
	ub.isBuildComplete = false
}
func (ub *ub受払投入実績) Count() int {
	return len(ub.config)
}
func (ub *ub受払投入実績) build(wb Wb受払投入実績) (s string, w string, execArgs []interface{}) {
	if ub.Count() == 0 {
		// 更新項目なし
		return
	}
...
func (ub *ub受払投入実績) copyMap() map[string]interface{} {
