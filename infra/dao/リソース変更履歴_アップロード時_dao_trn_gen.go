...
func (d daoTrnリソース変更履歴アップロード時) CountW(wb Wbリソース変更履歴アップロード時) (cnt int64, err error) {
	...
	prms, exists := where.Params()
	if exists {
		err = d.trn.QueryRow(fmt.Sprintf(sqlSelectリソース変更履歴アップロード時ForAggregation, "count(\"No\")", where.String()), prms...).Scan(&cnt)
		if err != nil {
			err = xerrors.Errorf(": %w", err)
			return
		}
		...
		err = d.trn.QueryRow(fmt.Sprintf(sqlSelectリソース変更履歴アップロード時ForAggregation, "count(\"No\")", "")).Scan(&cnt)
		if err != nil {
			err = xerrors.Errorf(": %w", err)
			return
		}
...
func (d daoTrnリソース変更履歴アップロード時) Insert(dr *Dtoリソース変更履歴アップロード時) (err error) {
	_, err = d.trn.Exec(sqlInsertリソース変更履歴アップロード時, dr.FldNo, dr.Fldアップロード履歴ID)
	if err != nil {
		err = xerrors.Errorf(": %w", err)
		return
	}
