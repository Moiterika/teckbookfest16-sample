...
func (d daoTrnログアップロード時) CountW(wb Wbログアップロード時) (cnt int64, err error) {
	...
	prms, exists := where.Params()
	if exists {
		err = d.trn.QueryRow(fmt.Sprintf(sqlSelectログアップロード時ForAggregation, "count(\"ID\")", where.String()), prms...).Scan(&cnt)
		if err != nil {
			err = xerrors.Errorf(": %w", err)
			return
		}
		...
		err = d.trn.QueryRow(fmt.Sprintf(sqlSelectログアップロード時ForAggregation, "count(\"ID\")", "")).Scan(&cnt)
		if err != nil {
			err = xerrors.Errorf(": %w", err)
			return
		}
...
func (d daoTrnログアップロード時) Insert(dr *Dtoログアップロード時) (err error) {
	_, err = d.trn.Exec(sqlInsertログアップロード時, dr.FldID, dr.Fldアップロード履歴ID)
	if err != nil {
		err = xerrors.Errorf(": %w", err)
		return
	}
