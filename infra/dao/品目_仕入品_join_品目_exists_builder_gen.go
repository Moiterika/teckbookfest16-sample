...
func (eb *eb品目仕入品join品目) buildEb品目仕入品(argCntStart ...int) (where Where) {
	where = eb.wb.build(argCntStart...)
	where.w = fmt.Sprintf(" AND EXISTS (SELECT * FROM \"品目\" WHERE \"品目\".\"ID\" = \"品目_仕入品\".\"ID\" %s)", where.w)
	return
}
