...
func (ub *ub品目仕入品) Set(field fld品目仕入品, val interface{}) *ub品目仕入品 {
	k := string(field)
...
func (ub *ub品目仕入品) Clear() {
	ub.config = make([]updateBuilderKvPair, 0)
	ub.keys = make(map[string]int)
	ub.isBuildComplete = false
}
func (ub *ub品目仕入品) Count() int {
	return len(ub.config)
}
func (ub *ub品目仕入品) build(wb Wb品目仕入品) (s string, w string, execArgs []interface{}) {
	if ub.Count() == 0 {
		// 更新項目なし
		return
	}
...
func (ub *ub品目仕入品) copyMap() map[string]interface{} {
