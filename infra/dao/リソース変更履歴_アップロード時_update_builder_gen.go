...
type ubリソース変更履歴アップロード時 struct {
	config          []updateBuilderKvPair
	keys            map[string]int
	isBuildComplete bool
}

func NewUbリソース変更履歴アップロード時() *ubリソース変更履歴アップロード時 {
	return &ubリソース変更履歴アップロード時{
		config:          make([]updateBuilderKvPair, 0),
		isBuildComplete: false,
		keys:            make(map[string]int),
	}
}
func (ub *ubリソース変更履歴アップロード時) Set(field fldリソース変更履歴アップロード時, val interface{}) *ubリソース変更履歴アップロード時 {
	k := string(field)
...
func (ub *ubリソース変更履歴アップロード時) Clear() {
	ub.config = make([]updateBuilderKvPair, 0)
	ub.keys = make(map[string]int)
	ub.isBuildComplete = false
}
func (ub *ubリソース変更履歴アップロード時) Count() int {
	return len(ub.config)
}
func (ub *ubリソース変更履歴アップロード時) build(wb Wbリソース変更履歴アップロード時) (s string, w string, execArgs []interface{}) {
	if ub.Count() == 0 {
		// 更新項目なし
		return
	}
...
func (ub *ubリソース変更履歴アップロード時) copyMap() map[string]interface{} {
