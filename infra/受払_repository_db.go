...
func (r *repDb受払) list() (list []*objects.Ent受払, err error) {
	dt受払, errDt := r.rm.dm.NewDaoDb受払().Dt()
	if errDt != nil {
	...
	for i, dr := range dt受払 {
		...
		dr仕入品単位, _ := dao単位.GetBy(dr仕入.Fld仕入単位ID)
		仕入数量, err仕入数量 := types.NewQuantity(dr仕入.Fld仕入数量, dr仕入品単位.Fldコード)
		if err仕入数量 != nil {
		...
		受払仕入, err受払仕入 := objects.NewVal受払仕入(仕入数量,
			...
			types.NewPrice(dr仕入.Fld仕入単価, dr仕入.Fld仕入通貨ID, dr仕入品単位.Fldコード),
		)
		if err受払仕入 != nil {
...
func (r *repDb受払) List() ([]*objects.Ent受払, error) {
...
func (r *repDb受払) GetBy(no objects.No) (e *objects.Ent受払, err error) {
...
func (r *repDb受払) AddNew(e *objects.Ent受払) error {
...
func (r *repDb受払) Save(アップロード履歴ID objects.No) (err error) {
	dao受払 := r.rm.dm.NewDaoDb受払()
	dao仕入 := r.rm.dm.NewDaoDb受払仕入()
	defer dao受払.Reset()
	defer dao仕入.Reset()
	...
	for _, e := range r.rm.list受払 {
		...
		if dr, errCode := dao受払.GetBy(dao.Id(e.GetNo)); !errors.Is(errCode, dao.NotFoundError) {
			...
			if e.Get仕入 != nil {
				仕入品単位, _ := dao単位.GetByCode(e.Get仕入.Get仕入数量.Unit())

				dr仕入 := &dao.Dto受払仕入{
					FldNo:     dr.FldNo,
					Fld仕入数量:   e.Get仕入.Get仕入数量.Val(),
					Fld仕入単位ID: 仕入品単位.FldID,
